    ...
    <div class="container">
      <article class="title-classic">
        ...
        <div class="title-classic-text">
          <p>Contactanos con cualquier duda sobre viajes y detalles</p>
        </div>
      </article>

      <form class="rd-form rd-form-variant-2 rd-mailform" method="post" action="contact-us.php">
        
        <div class="row row-14 gutters-14">
          <div class="col-md-4">
            <div class="form-wrap">
              <input class="form-input"  type="text" name="name1" required placeholder="Tu Nombre">
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-wrap">
              <input class="form-input"  type="email" name="email1" required placeholder="E-mail">
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-wrap">
              <input class="form-input"  type="tel" name="phone1" required placeholder="Celular">
            </div>
          </div>

          <div class="col-12">
            <div class="form-wrap">
              <label class="form-label" for="contact-message-2">Peticiones</label>
              <textarea class="form-input textarea-lg"  name="Comentario"></textarea>
            </div>
          </div>
...
if ($_POST) {
  ...
  if (empty($Email)) {
    echo "<h1>Falta ingresar el e-mail</h1>";
  }

  if (empty($celular)) {
    echo "<h1>Falta ingresar el celular</h1>";
  }


  if (empty($Comentario)) {
    echo "<h1>Falta ingresar tu comentario</h1>";
  }




  if ((!empty($Nombre)) && (!empty($Email)) && (!empty($celular)) && (!empty($Comentario))) {

  }

  include_once("Configuracion/funciones.php");
  Contacto($Nombre,$Email,$celular,$Comentario);
